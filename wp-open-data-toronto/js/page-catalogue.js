                                  ...
                                  '<div class="col-md-12 not-found">' +
                                    '<p>Please try again or ' +
                                      '<a href="mailto:opendata@toronto.ca?' +
                                        ...
                                        'Intended use:'+
                                      '"</a>let us know what data you want to see in the catalogue</p>' +
                                  '</div>' +
                                '</div>');
        return;
    } else {
...
function buildSidebar(response) {
    ...
    for (var i in results['search_facets']) {
        ...
        if (numFilters > config['filterSize']) {
            ...
            showMoreButton.html('<a href="#" role="button">Show ' + (numFilters - config['filterSize'] + 1) + ' more ' + $(sidebarEle).parents('.card').find('.card-header').text().trim().toLowerCase() + 's' + '</a>');
            showMoreButton.show();
        } else {
            showMoreButton.hide();
        }
...
function buildStaticUI() {
    ...
    $('.show-more').on('mouseenter mouseleave', function () {
        ...
        var labelText = $(this).siblings('li.hidden').length > 0 ? 'Show ' + ($(this).siblings('li').length - config['filterSize'] + 1) + ' more ' + filterTerm : 'Collapse<span class="sr-only">  expanded list of' + filterTerm + '</span>';
